...
def exportNeuroML2(
    ...
    for np_pop in list(net.pops.values()):
        ...
        for cell_name in list(net.params.cellParams.keys()):
            ...
            if sim.cfg.verbose:
                print("  -- Comparing conds: %s" % cell_param_set0)
        ...
        if (
            "cellModel" in np_pop.tags
            and not np_pop.tags["cellModel"] == "NetStim"
            and not str(cell_param_set) in cells_added
        ):
            if sim.cfg.verbose:
                print(
                ...
                for np_sec_name in list(cell_param_set["secs"].keys()):
                    ...
                    if "topol" in np_sec and len(np_sec["topol"]) > 0:
                        ...
                        if not (
                            ...
                            and np_sec["topol"]["childX"] == 0
                        ):
                            print(
                                "Currently only support cell topol with parentX == 1.0 and childX == 0"
                            )
                            exit(1)
    ...
    class NetPyNEBuilder(DefaultNetworkHandler):
        ...
        def handle_population(
            ...
            if isinstance(component_obj, Cell):
                ...
                cell = component_obj
                cellRule = {
                    "conds": {"cellType": component, "cellModel": component},
                    "secs": {},
                    "secLists": {},
                }
